...
## 2025-02-20 - Upgrade Non-deterministic RNG for Security
**Vulnerability:** Weak random number generation in `nondeterministicRng` and manual fallback RNG definitions relying purely on `Math.random()`, which can lead to predictable patterns and potential exploitation if non-deterministic entropy is strictly required.
**Learning:** `Math.random()` provides insufficient entropy and shouldn't be relied upon exclusively to seed PRNG engines when cryptographically secure randomness is available. Some fallback RNG definitions duplicate insecure randomness manually rather than using central utility functions.
**Prevention:** Implement `crypto.getRandomValues()` as the primary entropy source for RNG seeds (while maintaining `Math.random()` as a fallback for older/incompatible environments). Ensure centralized RNG utility functions like `nondeterministicRng` are used across the codebase instead of defining inline fallback RNG objects with `Math.random()`.
