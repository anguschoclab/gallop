...
## 2024-05-24 - [Replace Math.random() with generateUUID for ID Generation]
**Vulnerability:** Found insecure `Math.random().toString(36)` used for generating IDs in `src/core/npc/npcCycle.ts` which introduces predictability and collision risks (CWE-338). Also found a direct usage of `crypto.randomUUID()` in `src/core/stewards/stewardTypes.ts`.
**Learning:** Using `Math.random()` for IDs is not cryptographically secure and is prone to collisions. While `crypto.randomUUID()` is secure, calling it directly can lead to crashes in environments without a secure context (like older jsdom/Jest setups) where it isn't polyfilled.
**Prevention:** Always use the project's centralized `generateUUID()` utility from `@/core/uuid` for ID generation, which falls back gracefully and provides deterministic options when a seed is supplied.
