...
**Action:** When adding tooltips to explain disabled states, use the `DisabledTooltipWrapper` pattern or wrap the button in a focusable span with `tabIndex={0}` and `className="inline-block cursor-not-allowed"`, and add `pointer-events-none` to the disabled button.
