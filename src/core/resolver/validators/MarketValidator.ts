  ...
  validate(
    ...
    switch (intent.type) {
      ...
      case "claiming": {
        ...
        const race =
          cache?.raceMap?.get(claimingIntent.raceId) || state.races[claimingIntent.raceId];
        const horse =
          cache?.horseMap?.get(claimingIntent.horseId) || state.horses[claimingIntent.horseId];
