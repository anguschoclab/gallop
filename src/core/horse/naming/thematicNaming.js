...
exports.generateThematicName = generateThematicName;
var nameDatabase_1 = require("./nameDatabase");
var THEMES = {
    aggressive: {
        wordPools: [nameDatabase_1.RACING_TERMS, nameDatabase_1.ABSTRACT_TERMS],
        patterns: ["{W1} Surge", "Storm {W1}", "{W1} Force", "Bold {W1}"],
    },
    conservative: {
        wordPools: [nameDatabase_1.CLASSICAL_NAMES, nameDatabase_1.NATURE_TERMS],
        patterns: ["Noble {W1}", "{W1} Heritage", "Stable {W1}", "Classic {W1}"],
    },
    developer: {
        wordPools: [nameDatabase_1.NATURE_TERMS, nameDatabase_1.ABSTRACT_TERMS],
        patterns: ["{W1} Seed", "Future {W1}", "Young {W1}", "{W1} Potential"],
    },
    "win-now": {
        wordPools: [nameDatabase_1.RACING_TERMS, nameDatabase_1.ARISTOCRATIC_TITLES],
        patterns: ["{W1} Now", "Instant {W1}", "{W1} Victory", "Fast {W1}"],
    },
    specialist: {
        wordPools: [nameDatabase_1.CELESTIAL_TERMS, nameDatabase_1.RACING_TERMS],
        patterns: ["{W1} Expert", "Niche {W1}", "{W1} Focus", "Pure {W1}"],
    },
    breeder: {
        wordPools: [nameDatabase_1.CLASSICAL_NAMES, nameDatabase_1.ARISTOCRATIC_TITLES],
        patterns: ["{W1} Line", "Blood {W1}", "Legacy {W1}", "{W1} Heir"],
    },
    trader: {
        wordPools: [nameDatabase_1.ABSTRACT_TERMS, nameDatabase_1.RACING_TERMS],
        patterns: ["{W1} Deal", "Value {W1}", "Bargain {W1}", "Market {W1}"],
    },
    prestige: {
        wordPools: [nameDatabase_1.ABSTRACT_TERMS],
        patterns: ["{W1} Elite", "Royal {W1}", "Grand {W1}", "{W1} Excellence"],
    },
    generic: {
        wordPools: [nameDatabase_1.RACING_SPIRIT_ADJECTIVES, nameDatabase_1.RACING_SPIRIT_NOUNS],
 ...
 */
function generateThematicName(theme, rng) {
    var def = THEMES[theme];
    var pool = def.wordPools[rng.int(0, def.wordPools.length - 1)];
    var word = pool[rng.int(0, pool.length - 1)];
    var pattern = def.patterns[rng.int(0, def.patterns.length - 1)];
    var name = pattern;
    if (pattern.includes("{W1}")) {
        var w1 = def.wordPools[0][rng.int(0, def.wordPools[0].length - 1)];
        name = name.replace("{W1}", w1);
    }
    if (pattern.includes("{W2}")) {
        var w2 = def.wordPools[1][rng.int(0, def.wordPools[1].length - 1)];
