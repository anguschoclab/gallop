...
let cachedHorsesArray: Horse[] | null = null;
let cachedHorseRecord: Record<string, Horse> = {};

/**
 * Optimizes valuation loops by memoizing the array-to-record conversion.
 * If the exact same allHorses array reference is passed, the cached record is returned.
 * @param allHorses
 */
function getHorseRecord(allHorses: Horse[]): Record<string, Horse> {
  if (allHorses === cachedHorsesArray) return cachedHorseRecord;
  cachedHorsesArray = allHorses;
  cachedHorseRecord = {};
  for (let i = 0; i < allHorses.length; i++) {
    cachedHorseRecord[allHorses[i].id] = allHorses[i];
  }
  return cachedHorseRecord;
}

export function horsePriceWithPedigree(h: Horse, allHorses: Horse[]): number {
  const base = horsePrice(h);
  return (
    Math.round((base * pedigreeMultiplier(h, { horses: getHorseRecord(allHorses) })) / 50) * 50
  );
}
...
export function estimateBreedingValue(h: Horse, allHorses: Horse[] = []): number {
  ...
  const pedMul = pedigreeMultiplier(h, {
    horses: getHorseRecord(allHorses),
  });
...
export function horseCareerValuation(h: Horse, allHorses: Horse[] = []): HorseCareerValuation {
  ...
  const overall = calculateOverallRating(h);
  const potMod = 0.5 + (h.potential ?? 50) / 100;
  const pedMul = pedigreeMultiplier({ ...h, age: 1 }, { horses: getHorseRecord(allHorses) });
