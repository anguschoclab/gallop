...
export function runHeadToHeadSimulation(
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    const runners = horses.map((h, i) =>
      buildRunner(h, h.owned ?? false, distance, surface, { speedMul: 1, staminaDrainMul: 1 }, i + 1),
    );
    while (runners.length < 8) {
      const filler = generateHorse({ tier: "mid", owned: false }, rng);
      runners.push(buildRunner(filler, false, distance, surface, { speedMul: 1, staminaDrainMul: 1 }, runners.length + 1));
    }
  ...
  return horses.map((h) => {
    ...
    const avgPos = s.positions.length > 0 ? s.positions.reduce((a, b) => a + b, 0) / s.positions.length : 0;
