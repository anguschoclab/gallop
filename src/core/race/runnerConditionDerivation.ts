...
export function buildFieldContext(runners: Runner[]): FieldContext {
  ...
  for (let i = 0; i < runners.length; i++) {
    ...
    if (r.finishTime === null) {
      ...
      if (r.velocity > 0) {
        moving.push(r);
        movingVelocitySum += r.velocity;
        if (r.velocity > fastestVelocity) fastestVelocity = r.velocity;
      }
    }
  }

  const meanVelocity = moving.length > 0 ? movingVelocitySum / moving.length : 0;
