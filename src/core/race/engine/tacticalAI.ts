...
export function calculateTacticalAdjustment(
  ...
  // Use a single loop to avoid multiple filter/find passes in hot loop
  ...
  for (let i = 0; i < runners.length; i++) {
    ...
    // Check rival presence
    if (
      !nearbyRivalFound &&
      rivalHorseIds &&
      rivalHorseIds.length > 0 &&
      Math.abs(posDiff) < 6 &&
    ...
    if (posDiff > 0 && posDiff < 6 && Math.abs(r.lane - runner.lane) < 0.5) {
      if (posDiff < 3) {
        ...
        clusteredAheadCount++; // Beyond immediate traffic, within 6m
      }
    }
  }
