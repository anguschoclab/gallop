...
import type { ProgenyLeaderboard, ProgenyLeaderboardType, ProgenyRanking } from "./leaderboardTypes";
 ...
 */
function computeBeyerLeaderboard(
  runners: Horse[],
  currentDay: number
): ProgenyLeaderboard {
 ...
 */
function computeEarningsLeaderboard(
  runners: Horse[],
  currentDay: number
): ProgenyLeaderboard {
  const rankings = runners
 ...
 */
function computeStakesWinnersLeaderboard(
  runners: Horse[],
  currentDay: number
): ProgenyLeaderboard {
  const rankings = runners
    .map(h => ({
      ...
      sireId: h.pedigree?.sireId,
      sireName: h.sireName,
      value: h.raceHistory.filter(r => (r.grade || r.raceClass === "Stakes" || r.raceClass === "Group") && r.position === 1).length,
      metrics: {
        age: h.age,
        starts: h.careerStarts,
        wins: h.careerWins,
