/**
 * pregnancy.ts - Pregnancy resolution helpers
 *
 ...
 *
 * Dependencies: @/game/types (Horse, Pregnancy, Stable, RegionalSystem, BlueHenStatus, StudCareer),
 *   @/core/horse/horseFactory (resolveFoaling), @/core/race/naming/raceNameGenerator (getRegionalSystem),
 *   @/core/stable/stableConfig (PERSONALITY_CONFIG), @/core/common/formatting (formatCurrency),
 *   @/constants (BREEDING_FEE, GESTATION_DAYS, LIVE_FOAL_GUARANTEE_FEE)
 * Related files: src/game/store/slices/breedingSlice.ts (uses pregnancy helpers)
 */
...
/**
 * Core-local input type for pregnancy resolution.
 *
 * Replaces the previous `Pick<GameState, "horses" | "userSettings" | "reservedHorseNames">`
 * which forced core to import from the app layer. All three fields are
 * available from core types directly.
 */
export type PregnancyResolutionInput = {
 ...
 * @param state - Optional input with horses map, user settings, and reserved names
...
export function resolvePregnancies(
  ...
  usedNames: Set<string>,
  newDay: number,
  state?: PregnancyResolutionInput,
): PregnancyResult {
