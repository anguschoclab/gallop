...
export const REGIONAL_LINE_BIAS: Record<Bloodline, { surface?: "Turf" | "Dirt" | "Synthetic"; boost: number }> = {
...
export function computeCoiFromSnapshot(pedigree: Pedigree | undefined, maxDepth: number = 5): number {
...
function walkPedigree(p: Pedigree | undefined, depth: number, maxDepth: number, out: Map<string, number>) {
...
export function detectInbreedingPattern(pedigree: Pedigree | undefined): { ancestorId: string; sireGen: number; damGen: number } | undefined {
...
export function computeAhc(
  ...
  const parentAhc = ((sire.ancestralHistoryCoefficient ?? 0) + (dam.ancestralHistoryCoefficient ?? 0)) / 2;
  ...
  const provenSireBonus = (sire.coefficientOfInbreeding ?? 0) > 0.05 && sire.careerWins >= 3 ? 0.05 : 0;
  const provenDamBonus = (dam.coefficientOfInbreeding ?? 0) > 0.05 && dam.careerWins >= 3 ? 0.03 : 0;
