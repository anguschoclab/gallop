...
import type { PedigreeNode } from "@/lib/pedigreeGraph";

export const PedigreeNodeCard = memo(function PedigreeNodeCard({ data }: NodeProps<PedigreeNode>) {
  ...
  return (
    <>
      ...
      <div
        onClick={handleClick}
        className={cn(
          "rounded-sm border px-2 py-1.5 text-left transition-colors select-none",
          ...
          data.generation === 0 && "border-gold/40",
        )}
      ...
      >
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <span
            className={cn(
              "text-[10px] font-bold truncate text-cream leading-tight",
              !data.isReal && "italic",
            )}
            title={data.name}
          >
        ...
        <div className="flex items-center justify-between gap-1">
          <div className="flex items-center gap-1 text-[8px] font-mono text-cream/40 uppercase tracking-wider">
            {data.gender && (
              <span
                className={cn(
            ...
            <span>{data.isSire ? "Sire" : "Dam"}</span>
          </div>
