      ...
      <div className="bg-slate-900 border-2 border-gold/20 shadow-[0_0_50px_rgba(0,0,0,0.8)] max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-6 bg-black/40 border-b border-white/5 flex items-center justify-between">
           <div className="space-y-1">
              <div className="text-[10px] font-black uppercase text-gold/40 tracking-[0.3em]">Official_Resolution</div>
              <h2 className="text-2xl font-black text-cream uppercase tracking-tight font-[family-name:var(--font-display)]">{race.name}</h2>
           </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
          <div className="grid grid-cols-12 px-4 py-2 border-b border-white/5 font-mono text-[9px] uppercase font-black text-cream/20 tracking-widest">
             ...
             <div className="col-span-6">Asset / Rider</div>
          ...
          <div className="divide-y divide-white/5">
            {ordered.map((r, i) => {
              ...
              return (
                <div key={r.horseId} className="group py-4 px-4 hover:bg-white/[0.02] transition-colors">
                  <div className="grid grid-cols-12 items-center gap-4">
                    <div className="col-span-1">
                       <span className={cn(
                          "h-6 w-6 rounded-full flex items-center justify-center font-mono font-black text-[10px] tabular-nums",
                          i === 0 ? "bg-fame text-slate-950 shadow-[0_0_10px_rgba(212,175,55,0.4)]" : "bg-black/40 text-cream/40"
                       )}>
                          {i + 1}
                       </span>
                    </div>
                    ...
                    <div className="col-span-3 text-right">
                       {prize > 0 && (
                          <span className={cn(
                             ...
                             r.owned ? "text-success shadow-[0_0_8px_rgba(34,197,94,0.2)]" : "text-cream/20"
                          )}>
                  ...
                  {feedback && (
                    <div className="mt-3 ml-10 p-2 bg-blue-500/5 border border-blue-500/10 text-[10px] text-blue-400/80 font-mono italic leading-relaxed uppercase tracking-tight">
                       <span className="font-black not-italic text-blue-400 mr-2">LOG:</span> {feedback}
                    </div>
                  )}
                </div>
              );
