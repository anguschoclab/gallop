...
import { OUTPOST_CONSTANTS, type Outpost, type FacilityBranch } from "@/core/facilities/outpostTypes";
...
export function ImperialOutpostManager() {
  ...
  const outposts = (useGame((s) => (s as any).playerOutposts) as Outpost[]) || [
      {
          ...
          totalSlots: 12,
          facilities: {
              0: { type: "main_track", level: "elite", branch: "turf", maintenanceCost: 150, upgradeCost: 0, builtDay: 1 },
              4: { type: "barn", level: "premium", branch: "neutral", maintenanceCost: 60, upgradeCost: 0, builtDay: 1 },
              6: { type: "jockey_academy", level: "standard", branch: "neutral", maintenanceCost: 25, upgradeCost: 0, builtDay: 10 },
          },
  ...
  const selectedOutpost = outposts.find(o => o.id === (selectedOutpostId || outposts[0].id)) || outposts[0];
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
        {outposts.map(outpost => (
            <Button
                ...
                className={cn("gap-2 shrink-0", selectedOutpost.id === outpost.id ? "bg-gold text-t950" : "border-gold-muted text-cream-muted")}
        ...
        <Button variant="outline" className="gap-2 border-dashed border-gold-muted text-gold shrink-0">
        ...
        <Card className="lg:col-span-2 border-gold-muted bg-t900/50">
            <CardHeader>
                <div className="flex items-center justify-between">
                    <div>
                        <CardTitle className="font-[family-name:var(--font-display)]">Acreage Management</CardTitle>
                        <CardDescription>
            ...
            <CardContent>
                <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                    {Array.from({ length: selectedOutpost.totalSlots }).map((_, i) => {
                        ...
                        const footprint = facilityAtSlot ? OUTPOST_CONSTANTS.SLOT_FOOTPRINTS[facilityAtSlot.type] : 1;
                            ...
                            return (
                                <div 
                                    ...
                                    className={cn(
                                        ...
                                        facilityAtSlot.branch === "turf" ? "bg-green-900/40 border-green-700/50" : 
                                        facilityAtSlot.branch === "dirt" ? "bg-amber-900/40 border-amber-700/50" : 
                                        "bg-t800 border-gold-muted/30"
                                    )}
                                    style={{ gridColumn: `span ${Math.min(footprint, 2)}`, gridRow: `span ${Math.ceil(footprint / 2)}` }}
                                >
                                    ...
                                    <Badge className="h-3 text-[8px] px-1 bg-t700 text-gold-muted">{facilityAtSlot.level}</Badge>
                                </div>
                            );
                        }

                        return (
                            <div key={i} className="aspect-square rounded border border-dashed border-gold-muted/20 bg-t950/30 flex items-center justify-center group cursor-pointer hover:border-gold/50">
                                <span className="text-[10px] text-cream-muted group-hover:text-gold">Empty</span>
                            </div>
                        );
                    })}
                </div>
        ...
        <div className="space-y-4">
            <Card className="border-gold-muted">
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm uppercase tracking-wider text-cream-muted">Head Trainer</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <Card className="border-gold-muted">
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm uppercase tracking-wider text-cream-muted">Specialization</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                    ...
                    <Button variant="outline" className="w-full text-xs h-8 border-gold-muted text-gold hover:bg-gold hover:text-t950">
                        Manage Tech Tree
                    </Button>
                </CardContent>
            </Card>
            ...
            <Card className="border-gold-muted">
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm uppercase tracking-wider text-cream-muted">Logistics</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                    <div className="flex justify-between items-center text-xs">
