    ...
    <div className="space-y-6 pb-10 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gold/20 pb-6">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2 font-mono text-[10px] uppercase tracking-widest text-cream/40">
             ...
             <span className="w-1 h-1 rounded-full bg-white/20" />
             <span>Protocol: <span className="text-success font-black">STABLE</span></span>
          </div>
        </div>
        ...
        <Tabs value={selectedPeriod} onValueChange={(v) => setSelectedPeriod(v as PeriodKey)}>
          <div className="flex items-center justify-between bg-slate-900/40 p-1 border border-white/5 rounded-lg">
            <TabsList className="bg-transparent h-9">
              <TabsTrigger value="week" className="uppercase text-[9px] font-black tracking-widest data-[state=active]:bg-gold data-[state=active]:text-slate-950 h-full px-4">WEEK</TabsTrigger>
              <TabsTrigger value="month" className="uppercase text-[9px] font-black tracking-widest data-[state=active]:bg-gold data-[state=active]:text-slate-950 h-full px-4">MONTH</TabsTrigger>
              <TabsTrigger value="year" className="uppercase text-[9px] font-black tracking-widest data-[state=active]:bg-gold data-[state=active]:text-slate-950 h-full px-4">YEAR</TabsTrigger>
              <TabsTrigger value="allTime" className="uppercase text-[9px] font-black tracking-widest data-[state=active]:bg-gold data-[state=active]:text-slate-950 h-full px-4">ALL_TIME</TabsTrigger>
            </TabsList>
          </div>
        </Tabs>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="bg-slate-950 border border-gold/40 rounded-none shadow-2xl relative overflow-hidden group">
          ...
          <CardHeader className="pb-2">
            <CardDescription className="text-[9px] uppercase font-black tracking-[0.2em] text-gold-bright/60 flex items-center gap-1.5">
              <Wallet className="h-3 w-3" /> Liquid_Assets
            </CardDescription>
        ...
        <Card className="bg-slate-900/40 border border-white/5 rounded-none shadow-xl border-l-2 border-l-success/40">
          ...
          <CardContent className="pt-0 border-t border-white/5 bg-black/20 py-2">
            <div className="text-[8px] font-mono text-cream/20 uppercase tracking-widest">{activePeriodData.label}</div>
          </CardContent>
        </Card>
        ...
        <Card className={cn(
          ...
          <CardContent className="pt-0 border-t border-white/5 bg-black/20 py-2 flex items-center justify-between">
            ...
            <Badge variant={activePeriodData.netProfit >= 0 ? "default" : "destructive"} className="text-[8px] h-3.5 px-1 font-black uppercase rounded-none tracking-widest">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-4">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 mb-2 px-1">
             ...
             <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-cream/40">Revenue Analysis</h2>
          </div>
          <Card className="bg-slate-900/40 border-white/5 rounded-none shadow-xl">
             <CardContent className="p-6">
                {incomeBreakdown.length === 0 ? (
                  <div className="py-12 text-center text-[10px] font-mono text-cream/20 uppercase tracking-widest italic">Zero revenue data in current window</div>
                ) : (
                  <div className="space-y-4">
                    {incomeBreakdown.map((item) => (
                      <div key={item.label} className="space-y-1.5">
                        <div className="flex justify-between items-end">
                          <span className="text-[10px] font-bold text-cream/60 uppercase tracking-tight">{item.label}</span>
                          <span className="font-mono text-xs font-black text-success tabular-nums">{formatCurrency(item.amount)}</span>
                        </div>
          ...
          <Card className="bg-slate-900/40 border-white/5 rounded-none shadow-xl">
             <CardContent className="p-6">
                {expenseBreakdown.length === 0 ? (
                  <div className="py-12 text-center text-[10px] font-mono text-cream/20 uppercase tracking-widest italic">Zero expense data in current window</div>
                ) : (
                  <div className="space-y-4">
                    {expenseBreakdown.map((item) => (
                      <div key={item.label} className="space-y-1.5">
                        <div className="flex justify-between items-end">
                          <span className="text-[10px] font-bold text-cream/60 uppercase tracking-tight">{item.label}</span>
                          <span className="font-mono text-xs font-black text-destructive tabular-nums">{formatCurrency(item.amount)}</span>
                        </div>
                        <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                          <div
      ...
      <section className="space-y-4 pt-4">
        <div className="flex items-center gap-2 mb-2 px-1">
           ...
           <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-cream/40">System Audit Ledger</h2>
        </div>
        <Card className="bg-slate-900/20 border-white/5 rounded-none shadow-2xl overflow-hidden">
          <CardHeader className="bg-black/40 py-3 border-b border-white/10 flex flex-row items-center justify-between">
            <div className="text-[10px] font-black uppercase tracking-[0.2em] text-gold-muted/60 px-4">Cash Movement Log</div>
            <div className="flex items-center gap-4 px-4">
               <span className="text-[10px] font-mono text-cream/20 uppercase">SAMPLE_50</span>
               <Badge variant="outline" className="border-white/10 text-cream/40 font-mono text-[9px] h-5 rounded-none">{transactions.length} RECS</Badge>
            </div>
          </CardHeader>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                  {recentTransactions.map((t, idx) => (
                    <tr key={t.id || idx} className="group hover:bg-white/[0.02] transition-colors relative">
                      <td className="px-8 py-3 font-mono text-[10px] text-cream/20 group-hover:text-gold/40 transition-colors tabular-nums">D{String(t.day).padStart(3, '0')}</td>
                      ...
                      <td className="px-8 py-3 text-[11px] font-mono text-cream/40 group-hover:text-cream/60 transition-colors uppercase italic truncate max-w-xs">{t.description}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
