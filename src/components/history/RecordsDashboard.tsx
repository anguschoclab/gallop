  ...
  return (
    <div className="p-6 space-y-6 bg-[#0a0a0a] min-h-screen text-cream">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-cream-muted italic">Preserving the legacy of the turf for future generations.</p>
      </header>

      <Tabs defaultValue="hof" className="w-full">
        <TabsList className="bg-black/40 border border-white/10 p-1 mb-6">
          <TabsTrigger value="hof" className="data-[state=active]:bg-gold data-[state=active]:text-black">
            <Star size={16} className="mr-2" />
            Hall of Fame
          </TabsTrigger>
          <TabsTrigger value="season" className="data-[state=active]:bg-gold data-[state=active]:text-black">
        ...
        <TabsContent value="hof">
          <div className="flex flex-col gap-6">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-cream-muted" size={18} />
              <Input 
            ...
            {filteredHOF.length === 0 ? (
              <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-xl">
                ...
                <p className="text-cream-muted">No legends have been inducted yet. Win 3 Grade 1s to make history.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredHOF.sort((a,b) => b.inductionDay - a.inductionDay).map(entry => (
                  <Card key={entry.horseId} className="bg-black/60 border-gold/20 hover:border-gold/50 transition-all overflow-hidden group">
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div className="bg-white/5 p-2 rounded">
                          ...
                          <p className="font-bold text-gold">${(entry.lifetimeEarnings / 1000).toFixed(0)}k</p>
                        </div>
                        <div className="bg-white/5 p-2 rounded">
                      ...
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase text-gold font-bold tracking-widest">Achievements</p>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="text-sm">
                      {seasonRecords.sort((a,b) => b.day - a.day).map(record => (
                        <tr key={record.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-2">
                              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: record.winnerSilk }} />
                              <Link to="/stable/$horseId" params={{ horseId: record.winnerId }} className={record.isPlayerOwned ? "font-bold text-gold" : ""}>
                                {record.winnerName}
                              </Link>
                            </div>
                          </td>
