    ...
    <div className="space-y-4">
      ...
      {program.history.length > 0 && (
        <Card className="border-gold-muted">
          ...
          <CardContent>
            <div className="space-y-1">
              {[...program.history].reverse().slice(0, 6).map((entry, i) => {
      ...
      <Card className="border-gold-muted">
        <CardHeader>
          ...
          <p className="text-sm text-cream-muted font-[family-name:var(--font-body)]">
            A breeding program gives you a generational target. The simulator will
            highlight stallions that move your line closer to the archetype.
          </p>
        </CardHeader>
            ...
            {(["all", "classic", "turf", "dirt", "triple"] as const).map((f) => (
              <button
                ...
                className={cn(
                  ...
                  filter === f
                    ? "bg-gold text-t950"
                    : "bg-t700 text-cream-muted hover:bg-t600",
                )}
              >
