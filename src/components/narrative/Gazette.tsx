      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gold/20 pb-6">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2 font-mono text-[10px] uppercase tracking-widest text-cream/40">
             ...
             <span>Edition: <span className="text-gold">D{String(day).padStart(3, '0')}</span></span>
             <span className="w-1 h-1 rounded-full bg-white/20" />
             <span>Security: <span className="text-success font-black">UNCLASSIFIED</span></span>
          </div>
        </div>
        ...
        <main className="lg:col-span-8 space-y-6">
          {mainNews.length > 0 ? (
            ...
            <div className="p-32 text-center border-2 border-dashed border-white/5 bg-black/10">
               ...
               <p className="font-bold text-cream/40 uppercase tracking-[0.3em] font-[family-name:var(--font-display)]">Quiet Day at the Tracks</p>
               <p className="text-[10px] font-mono text-cream/10 uppercase mt-2">The racing world is catching its breath. No major results reported today.</p>
            </div>
          )}
        </main>
                ...
                <div className="divide-y divide-white/5">
                   {sideNews.length > 0 ? (
                    sideNews.map((item, index) => (
                      <div key={`${item.id}-${index}`} className="p-4 space-y-2 hover:bg-white/[0.02] transition-colors group">
                        <div className="flex items-center justify-between">
                           <Badge variant="outline" className="text-[8px] h-3.5 px-1 font-black uppercase border-white/10 text-cream/30 rounded-none tracking-widest">
                              {item.category}
                           </Badge>
                           <span className="text-[9px] font-mono text-cream/10">D{String(item.day).padStart(3, '0')}</span>
                        </div>
                    ...
                    <div className="p-12 text-center text-[10px] font-mono text-cream/20 uppercase tracking-widest italic">Stable conditions across all regional tracks.</div>
                  )}
                </div>
              </CardContent>
            </Card>
          ...
          <section className="bg-gold/5 border-2 border-double border-gold/10 p-6 space-y-4 relative overflow-hidden">
            ...
            <div className="space-y-3 relative z-10">
               <div className="p-3 bg-black/40 border border-white/5 space-y-1">
                  <div className="text-[8px] font-black text-gold/40 uppercase tracking-widest">Real Estate</div>
                  <p className="text-[10px] text-cream/60 leading-tight italic uppercase">Looking for prime pasture? Contact the Kentucky Syndicate for luxury acreage.</p>
               </div>
               <div className="p-3 bg-black/40 border border-white/5 space-y-1">
                  <div className="text-[8px] font-black text-gold/40 uppercase tracking-widest">Personnel</div>
                  <p className="text-[10px] text-cream/60 leading-tight italic uppercase">Elite exercise riders available for short-term seasonal contracts in the UK.</p>
               </div>
            </div>
          </section>
        </aside>
...
const NewsArticle: React.FC<{ item: NewsItem }> = ({ item }) => {
  ...
  return (
    <Card className={cn(
      ...
      isHigh ? "bg-slate-900/60 border-l-4 border-l-gold" : "bg-slate-900/20 border-l-4 border-l-slate-700"
    )}>
       <CardHeader className="pb-3 border-b border-white/5 flex flex-row items-center justify-between bg-black/20">
          ...
          <span className="text-[10px] font-mono text-cream/20">D{String(item.day).padStart(3, '0')}</span>
       </CardHeader>
       <CardContent className={cn("p-8", isHigh ? "space-y-4" : "space-y-3")}>
          ...
          <div className="pt-4 flex justify-between items-center border-t border-white/5">
             <div className="text-[9px] font-black uppercase text-cream/20 tracking-widest">Source: Internal Frequency 1.09</div>
             <Link to="/gazette" className="text-[10px] font-black text-gold/40 hover:text-gold transition-colors uppercase tracking-widest flex items-center gap-1 group/link">
                Intelligence Brief <ChevronRight className="h-3 w-3 group-hover/link:translate-x-0.5 transition-transform" />
             </Link>
          </div>
       </CardContent>
    </Card>
