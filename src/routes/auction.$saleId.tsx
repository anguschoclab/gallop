  ...
  if (!sale) {
    return (
      <div className="p-12 text-center space-y-4">
        <h1 className="text-4xl font-black font-[family-name:var(--font-display)] text-cream">SALE_NOT_FOUND</h1>
        <Button variant="ghost" onClick={() => navigate({ to: "/auction" })} className="text-gold uppercase font-mono text-xs tracking-widest hover:underline">
          ← Return to Exchange
        </Button>
      </div>
    );
    ...
    <div className="space-y-6 pb-20 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gold/20 pb-6">
        <div>
          ...
          <div className="flex items-center gap-3">
             ...
             <Badge className={cn("rounded-none font-black text-[10px] tracking-widest px-3 h-6", 
               isResolved ? "bg-slate-800 text-cream/40" : "bg-gold text-slate-950 shadow-[0_0_15px_rgba(212,175,55,0.3)]"
             )}>
          ...
          <div className="flex items-center gap-3 mt-2 font-mono text-[10px] uppercase tracking-widest text-cream/40">
             <span>Market Class: <span className="text-gold-muted">{KIND_LABELS[sale.kind] ?? sale.kind}</span></span>
             <span className="w-1 h-1 rounded-full bg-white/20" />
             <span>Lot Count: <NumericValue value={activeLots.length} /></span>
             <span className="w-1 h-1 rounded-full bg-white/20" />
             <span>System Day: <NumericValue value={sale.day} /></span>
          </div>
        </div>
        
        <div className="flex flex-col items-end gap-1">
           <div className="text-[10px] font-mono text-cream/20 uppercase tracking-widest">Available_Capital</div>
           <div className="text-2xl font-black font-mono text-success tabular-nums tracking-tighter">{formatCurrency(cash)}</div>
        </div>
      </div>
      ...
      {isSaleDay && !isResolved ? (
        ...
        <div className="p-32 text-center border-2 border-dashed border-white/5 bg-black/10">
           ...
           <p className="font-bold text-cream/40 uppercase tracking-[0.3em] font-[family-name:var(--font-display)]">No Catalog Data Detected</p>
           <p className="text-[10px] font-mono text-cream/10 uppercase mt-2">Historical data for this window is unavailable.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
           ...
           <aside className="lg:col-span-4 space-y-8">
              {!isResolved && (
                <section className="space-y-4">
                   <div className="flex items-center gap-2 mb-2 px-1">
                      ...
                      <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-cream/40">Catalog Search</h2>
                   </div>
              ...
              <section className="space-y-4">
                 <div className="flex items-center gap-2 mb-2 px-1">
                    ...
                    <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-cream/40">Catalog Index</h2>
                 </div>
                    ...
                    <div className="divide-y divide-white/5">
                       {displayLots.map((lot, idx) => {
                          ...
                          return (
                            <div 
                              key={lot.id} 
                              onClick={() => { setLotIndex(idx); setMessage(""); }}
                              className={cn(
                                "px-4 py-3 cursor-pointer transition-all flex items-center justify-between group",
                                isActive ? "bg-gold/10 border-l-2 border-l-gold" : "hover:bg-white/[0.02] border-l-2 border-l-transparent"
                              )}
                            >
                               <div className="space-y-0.5">
                                  <div className="flex items-center gap-2">
                                     <span className="text-[9px] font-mono text-cream/20 group-hover:text-gold/40 transition-colors">#{String(idx + 1).padStart(3, '0')}</span>
                                     <span className={cn("text-xs font-bold uppercase tracking-tight transition-colors", isActive ? "text-gold" : "text-cream/60 group-hover:text-cream")}>
                               ...
                               <div className="text-right">
                                  {lot.hammerPrice && (
                                     <div className={cn("text-[10px] font-mono font-bold tabular-nums", lot.passed ? "text-destructive/40" : "text-success/60")}>
                                        {formatCurrency(lot.hammerPrice)}
                                     </div>
                                  )}
                                  {lot.passed && <span className="text-[8px] font-black uppercase text-destructive/60 tracking-widest leading-none">PASSED</span>}
                               </div>
                            </div>
                          );
                       })}
           ...
           <main className="lg:col-span-8 space-y-8">
              ...
              {displayLots.length > 0 && currentLot && horse ? (
                     ...
                     <div className="bg-black/40 px-8 py-4 border-b border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                           <div className="text-center bg-gold/10 border border-gold/20 p-2 min-w-[60px]">
                              <div className="text-[8px] font-black uppercase text-gold/40 tracking-tighter leading-none mb-1">Lot_ID</div>
                              <div className="text-xl font-black font-mono text-gold leading-none tabular-nums">#{String(lotIndex + 1).padStart(3, '0')}</div>
                           </div>
                           <div className="space-y-0.5">
                              <h2 className="text-4xl font-black text-cream font-[family-name:var(--font-display)] uppercase tracking-tight group-hover:text-gold transition-colors">{horse.name}</h2>
                              <div className="flex items-center gap-3 font-mono text-[10px] uppercase tracking-widest text-cream/40">
                                 <span>{genderSymbol(horse.gender)} {horse.gender}</span>
                                 <span className="w-1 h-1 rounded-full bg-white/20" />
                        ...
                        <div className="flex items-center gap-2">
                           {currentLot.passed && <Badge variant="secondary" className="rounded-none font-black text-[10px] tracking-widest h-8 px-4 bg-destructive/10 text-destructive border-destructive/20 uppercase">PASSED_RESERVE</Badge>}
                           {!currentLot.passed && !isResolved && isPlayerLeading && (
                             <Badge className="bg-success text-slate-950 rounded-none font-black text-[10px] tracking-widest h-8 px-4 animate-pulse shadow-[0_0_15px_rgba(34,197,94,0.3)]">LEADING_BIDER</Badge>
                           )}
                           {!currentLot.passed && currentLot.soldToStableId && isResolved && (
                             <div className="text-right">
                                <div className="text-[8px] font-black uppercase text-cream/20 tracking-widest mb-1">Acquired_By</div>
                                <Badge variant="outline" className="border-gold/30 text-gold font-black text-[9px] uppercase tracking-widest h-6 rounded-none px-3 bg-gold/5">
                                   {stables.find((s) => s.id === currentLot.soldToStableId)?.name?.toUpperCase() ?? "NPC_ENTITY"}
                                </Badge>
                             </div>
                           )}
                        </div>
                           ...
                           <div className="md:col-span-5 space-y-6">
                              <div className="aspect-square bg-slate-950 border border-white/5 p-4 flex items-center justify-center relative shadow-inner">
                                 ...
                                 <div className="absolute bottom-4 left-4 bg-black/80 border border-white/10 px-2 py-1 flex items-center gap-2 shadow-xl">
                                    ...
                                    <span className="text-[8px] font-mono text-gold-muted/60 uppercase tracking-widest">Asset_Verified</span>
                                 </div>
                              </div>
                              ...
                              <div className="space-y-4">
                                 <div className="flex items-center gap-2 mb-1 px-1">
                                    ...
                                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-cream/40">Pedigree Analysis</h3>
                                 </div>
                                 <Card className="bg-black/20 border-white/5 rounded-none border-l border-l-pink-500/30">
                                    <CardContent className="p-4 space-y-3">
                                       <div className="flex items-start gap-4">
                                          <div className="space-y-0.5 flex-1">
                                             <div className="text-[8px] font-black uppercase text-pink-500/40 tracking-widest">Sire Line</div>
                                             <div className="text-xs font-bold text-cream uppercase">{horse.sireName || 'UNKNOWN'}</div>
                                          </div>
                                          <div className="space-y-0.5 flex-1 border-l border-white/5 pl-4">
                                             <div className="text-[8px] font-black uppercase text-pink-500/40 tracking-widest">Dam Line</div>
                                             <div className="text-xs font-bold text-cream uppercase">{horse.damName || 'UNKNOWN'}</div>
                                          </div>
                                       </div>
                                       {horse.bruceLoweFamily && (
                                          <div className="pt-2 border-t border-white/5">
                                             <div className="text-[8px] font-black uppercase text-cream/20 tracking-widest"> Bruce Lowe Family</div>
                                             <div className="text-xs font-mono font-bold text-pink-400">FAMILY_0{horse.bruceLoweFamily}</div>
                                          </div>
                                       )}
                                    </CardContent>
                                 </Card>
                           ...
                           <div className="md:col-span-7 space-y-8">
                              <section className="space-y-4">
                                 <div className="flex items-center gap-2 mb-1 px-1">
                                    ...
                                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-cream/40">Technical Specifications</h3>
                                 </div>
                                 <div className="grid grid-cols-2 gap-4">
                                    {(['conformation', 'temperament', 'coatColor', 'runningStyle'] as const).map(spec => (
                                 ...
                                 {displayStats && (
                                    <div className="pt-4 border-t border-white/5 space-y-3">
                                       <div className="flex items-center justify-between">
                                          <div className="text-[8px] font-black uppercase text-gold/40 tracking-widest">Scouting_Intel_Readout</div>
                                          {displayStatsResult?.overallEstimate && (
                                             <Badge variant="outline" className="border-gold/20 text-gold font-mono text-[9px] h-5 rounded-none px-2 uppercase">Est_OVR: {displayStatsResult.overallEstimate}</Badge>
                                          )}
                                       </div>
                                       <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                                          {Object.entries(displayStats).map(([key, val]) => (
                                             <div key={key} className="flex items-center justify-between border-b border-white/5 pb-1">
                                                <span className="text-[9px] font-mono text-cream/20 uppercase tracking-tighter">{key}</span>
                                                <span className="text-xs font-mono font-black text-cream/60 tabular-nums">{val as string}</span>
                                             </div>
                                          ))}
                                       </div>
                                    </div>
                              ...
                              {!isResolved && !currentLot.passed && (
                                 <section className="space-y-4 pt-4">
                                    <div className="flex items-center gap-2 mb-1 px-1">
                                       ...
                                       <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-success/60">Exchange Terminal</h3>
                                    </div>
                                    <div className="bg-black/40 border border-success/20 p-6 space-y-6">
                                       <div className="flex items-center justify-between border-b border-white/5 pb-4">
                                          <div className="space-y-0.5">
                                             <div className="text-[8px] font-black uppercase text-cream/20 tracking-widest">Market Valuation</div>
                                          ...
                                          <div className="text-right space-y-0.5">
                                             <div className="text-[8px] font-black uppercase text-cream/20 tracking-widest">Required Step</div>
                                       ...
                                       <div className="space-y-4">
                                          ...
                                          <div className="grid grid-cols-2 gap-4">
                                             <div className="space-y-1.5">
                                                <label className="text-[8px] uppercase font-black text-cream/20 tracking-widest px-1">Manual_Offer</label>
                                                <div className="flex gap-2">
                                                   <Input
                                                      placeholder="AMOUNT..."
                                                      value={bidInput}
                                             ...
                                             <div className="space-y-1.5">
                                                <label className="text-[8px] uppercase font-black text-cream/20 tracking-widest px-1">Auto_Cap_Limit</label>
                                                <div className="flex gap-2">
                                                   <Input
                                                      placeholder="LIMIT..."
                                                      value={maxBid}
                                       ...
                                       {message && (
                                          <div className={cn(
                                             ...
                                             message.includes("placed") ? "text-success bg-success/5" : "text-destructive bg-destructive/5"
                                          )}>
                                             {message}
                                          </div>
                                       )}
                              ...
                              {isResolved && currentLot.hammerPrice && !currentLot.passed && (
                                 <div className="p-6 bg-gold/5 border-2 border-double border-gold/20 text-center space-y-2">
                                    <div className="text-[9px] font-black uppercase text-gold tracking-[0.4em]">Transaction_Resolution</div>
                                    ...
                                    <div className="text-[10px] font-mono text-gold-muted uppercase tracking-widest">
                                       Acquired By: {currentLot.soldToStableId ? (stables.find((s) => s.id === currentLot.soldToStableId)?.name?.toUpperCase() ?? "NPC_ENTITY") : "PLAYER_STABLE"}
                                    </div>
                                 </div>
                              )}
                           </div>
                    ...
                    <Card className="bg-slate-900/40 border-destructive/20 rounded-none shadow-xl border-t-2 border-t-destructive/40">
                       <CardContent className="p-6 flex items-center justify-between gap-6">
                          <div className="space-y-1">
                             ...
                             <p className="text-[10px] font-mono text-cream/30 uppercase tracking-tighter">Emergency decommissioning of asset from public catalog.</p>
                          </div>
                          {sale.day > day ? (
                             <AlertDialog>
                               <AlertDialogTrigger asChild>
                                 <Button variant="destructive" className="h-10 px-6 font-black uppercase text-[10px] tracking-widest rounded-none">
                                   DECOMMISSION_ASSET
                                 </Button>
                               </AlertDialogTrigger>
                               <AlertDialogContent className="bg-slate-950 border-destructive/30 rounded-none">
                                 <AlertDialogHeader>
                                   <AlertDialogTitle className="text-destructive uppercase font-black tracking-widest">Confirm Withdrawal</AlertDialogTitle>
                                   <AlertDialogDescription className="text-cream/60 font-mono text-xs uppercase tracking-tighter">
                                     Removal of <strong>{horse.name}</strong> from catalog. No transaction proceeds will be generated.
                                   </AlertDialogDescription>
                                 </AlertDialogHeader>
                                 <AlertDialogFooter>
                                   <AlertDialogCancel className="rounded-none font-black text-[10px] uppercase tracking-widest border-white/10 hover:bg-white/5">Cancel</AlertDialogCancel>
                                   <AlertDialogAction
                             ...
                             <Button disabled variant="destructive" className="h-10 px-6 font-black uppercase text-[10px] tracking-widest rounded-none opacity-20">
                ...
                <div className="p-32 text-center border-2 border-dashed border-white/5 bg-black/10">
                   <hardDrive className="h-16 w-16 mx-auto mb-6 text-cream/5" />
                   <p className="font-bold text-cream/40 uppercase tracking-[0.3em] font-[family-name:var(--font-display)]">Asset Sync Failure</p>
                   <p className="text-[10px] font-mono text-cream/10 uppercase mt-2">Central registry cannot identify lot data.</p>
                </div>
              ) : null}
              ...
              {isResolved && (
                <Card className="bg-slate-900/40 border-white/5 rounded-none shadow-xl border-t-2 border-t-gold/40">
                  <CardHeader className="bg-black/20 border-b border-white/5">
                    <CardTitle className="text-[10px] font-black uppercase tracking-[0.4em] text-gold-muted/60">Sale Exchange Summary</CardTitle>
                  </CardHeader>
                  <CardContent className="p-8">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                       <div className="space-y-1">
                          <div className="text-[9px] font-black uppercase text-cream/20 tracking-widest">Total Lots</div>
                          <div className="text-xl font-black font-mono text-cream tabular-nums">{activeLots.length}</div>
                       </div>
                       <div className="space-y-1">
                          <div className="text-[9px] font-black uppercase text-cream/20 tracking-widest">Assets Sold</div>
                          <div className="text-xl font-black font-mono text-success tabular-nums">{activeLots.filter((l) => !l.passed && l.hammerPrice).length}</div>
                       </div>
                       <div className="space-y-1">
                          <div className="text-[9px] font-black uppercase text-cream/20 tracking-widest">Reserve Passed</div>
                          <div className="text-xl font-black font-mono text-destructive/60 tabular-nums">{activeLots.filter((l) => l.passed).length}</div>
                       </div>
                       {(() => {
                        ...
                        return (
                          <div className="space-y-1">
                             <div className="text-[9px] font-black uppercase text-gold/40 tracking-widest">Top Valuation</div>
                             <div className="text-xl font-black font-mono text-gold-bright tabular-nums">{formatCurrency(top.hammerPrice!)}</div>
                             <div className="text-[8px] font-mono text-gold-muted/40 uppercase tracking-tighter truncate">{topHorse?.name}</div>
                          </div>
                        );
                      })()}
                    </div>
