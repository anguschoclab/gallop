    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-gold-muted">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-cream-muted font-[family-name:var(--font-body)]">
              Cash on hand
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-gold-muted">
          ...
          <CardContent>
            <div className="flex items-baseline gap-2">
              ...
              {hotyCount > 0 && (
                <span className="text-xs text-fame font-medium font-[family-name:var(--font-mono)] tabular-nums">
                  {hotyCount} HOTY
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {gradeData.map(({ grade, owned }) => (
                <div key={grade} className="text-center">
                  <div
                    className={cn(
                      "inline-flex items-center px-2 py-0.5 rounded border text-[10px] font-semibold mb-1",
          ...
          {recentReputationEvents.length === 0 ? (
            ...
            <div className="space-y-1">
              {recentReputationEvents.map((event, i) => (
                <div
                  key={i}
                  className="text-sm flex gap-3 border-b border-gold-muted/30 last:border-0 py-1 font-[family-name:var(--font-body)]"
                >
                  <span className="text-cream-muted font-[family-name:var(--font-mono)] tabular-nums w-10 shrink-0">
                    D{event.day}
                  </span>
          ...
          <CardContent className="space-y-2">
            {horses
              ...
              .map((h) => (
                <Link
                  ...
                  to="/stable/$horseId"
                  params={{ horseId: h.id }}
                  className="flex items-center gap-3 p-2 rounded hover:bg-t700 transition-colors"
                >
            ...
            {horses.length === 0 && (
              <p className="text-sm text-cream-muted italic">
                No horses in training. Visit the auction to acquire stock.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-gold-muted">
          ...
          <CardContent className="space-y-2">
            {upcoming.map((r) => (
              <div
                key={r.id}
                className="flex items-center justify-between p-2 rounded hover:bg-t700"
              >
                <div>
                  <p className="font-medium text-sm text-cream font-[family-name:var(--font-display)]">
                    {r.name}
            ...
            {upcoming.length === 0 && (
              <p className="text-sm text-cream-muted italic">No races today.</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-gold-muted">
        ...
        <CardContent className="space-y-1">
          ...
          {log.length === 0 && (
            <p className="text-sm text-cream-muted italic">No recent activity.</p>
          )}
        </CardContent>
      </Card>
    </div>
