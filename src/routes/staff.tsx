...
function StaffManagement() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-cream font-[family-name:var(--font-display)]">Stable Staff</h1>
        <p className="text-cream-muted">Manage your specialized team to improve horse performance and health.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card key={staff.id} className="border-gold-muted overflow-hidden">
                  <div className="flex">
                    <div className={cn(
                      ...
                      staff.tier === 'elite' ? "bg-fame" : staff.tier === 'mid' ? "bg-gold" : "bg-t700"
                    )}>
                      ...
                      <div>
                        ...
                        {staff.traits.length > 0 && (
                          <div className="flex gap-1 mt-2">
                            {staff.traits.map(t => (
                              <Badge key={t} className="bg-t800 text-[9px] text-cream-muted">{t}</Badge>
                            ))}
                          </div>
                        )}
                      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            {staffPool.map((staff) => (
              <Card key={staff.id} className="border-gold-muted/50 hover:border-gold transition-colors">
                <CardContent className="p-4 flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-cream">{staff.name}</h3>
                      <Badge variant="secondary" className="text-[10px] h-4 px-1">{STAFF_TIER_LABELS[staff.tier]}</Badge>
                    </div>
                    <p className="text-xs text-gold font-medium uppercase tracking-wider">{STAFF_ROLE_LABELS[staff.role]}</p>
                  ...
                  <Button size="sm" onClick={() => handleHire(staff.id)}>Hire</Button>
                </CardContent>
              </Card>
            ))}
            {staffPool.length === 0 && (
              <p className="text-sm text-cream-muted italic text-center py-4">No candidates available today. Check back tomorrow.</p>
            )}
          </div>
        </div>
      </div>
