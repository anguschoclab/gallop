...
import { Gavel, Clock, CheckCircle, Sparkles, Calendar as CalendarIcon, ArrowRight, ShieldCheck, HardDrive, Package, ExternalLink } from "lucide-react";
        ...
        <aside className="lg:col-span-4 space-y-8 lg:sticky lg:top-6">
          ...
          <section className="space-y-4">
             <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-cream/40">Deployment Dock</h2>
             </div>
             <Card className="bg-slate-900/40 border-white/5 rounded-none shadow-xl border-l-2 border-l-gold/40">
                <CardHeader className="pb-2 border-b border-white/5 bg-black/20">
                   <div className="flex justify-between items-center">
                      <CardTitle className="text-[9px] font-black uppercase tracking-widest text-cream/40">Eligible Assets</CardTitle>
                      <span className="text-[9px] font-mono text-gold-bright font-black uppercase">POOL: {consignablePairs.length}</span>
                   </div>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="divide-y divide-white/5 max-h-[400px] overflow-y-auto custom-scrollbar">
                      ...
                      <div key={horse.id} className="p-4 hover:bg-white/[0.02] transition-colors group">
                        <div className="flex items-center justify-between gap-3 mb-2">
                           <div className="flex items-center gap-2">
                              ...
                              <span className="font-bold text-cream uppercase tracking-tight text-xs group-hover:text-gold transition-colors">{horse.name}</span>
                           </div>
                           <Badge variant="outline" className="h-4 text-[8px] border-white/10 text-cream/40 font-mono">D{sale.day}</Badge>
                        </div>
                    ...
                    {consignablePairs.length === 0 && (
                      <div className="p-12 text-center text-[10px] font-mono text-cream/20 uppercase tracking-widest italic">No assets ready for deployment</div>
                    )}
                  </div>
                </CardContent>
                <div className="p-3 bg-black/40 border-t border-white/5 text-center">
                   <p className="text-[8px] font-mono text-cream/20 uppercase">House Commission Fixed at {Math.round(CONSIGNMENT_COMMISSION * 100)}%</p>
                </div>
             </Card>
          </section>
          ...
          {past.length > 0 && (
            <section className="space-y-4">
               <div className="flex items-center gap-2 mb-2 px-1">
                  ...
                  <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-cream/40">Transaction Archive</h2>
               </div>
               <div className="space-y-2">
                  {past.slice(0, 5).map((sale) => {
                    ...
                    return (
                      <Link key={sale.id} to="/auction/$saleId" params={{ saleId: sale.id }} className="block group">
                        <div className="p-3 border border-white/5 bg-slate-900/20 group-hover:border-white/20 transition-all">
                           <div className="flex justify-between items-center mb-1">
                              <span className="text-[10px] font-bold text-cream/60 group-hover:text-cream transition-colors uppercase truncate max-w-[140px]">{sale.name}</span>
        ...
        <main className="lg:col-span-8 space-y-8">
          ...
          {todaysSales.length > 0 && (
            <section className="space-y-4">
               <div className="flex items-center gap-2 mb-2 px-1">
                  ...
                  <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-success">Live Exchange Floor</h2>
               </div>
               <div className="grid gap-4">
                  {todaysSales.map((sale) => (
                    <Card key={sale.id} className="bg-slate-900/60 border-success/30 rounded-none shadow-2xl relative overflow-hidden">
                       ...
                       <CardContent className="p-8 flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
                          <div className="space-y-2 text-center md:text-left">
                             <Badge className="bg-success text-slate-950 font-black uppercase text-[10px] tracking-[0.2em] rounded-none px-3">Active_Ring</Badge>
                             <h3 className="text-4xl font-black text-cream font-[family-name:var(--font-display)] uppercase tracking-tight">{sale.name}</h3>
                             <p className="text-xs font-mono text-cream/40 uppercase tracking-tighter">
                                {KIND_LABELS[sale.kind] ?? sale.kind} · <NumericValue value={sale.lots.filter((l) => !l.withdrawn).length} /> Assets Catalogued
                             </p>
                          </div>
                          <Link to="/auction/$saleId" params={{ saleId: sale.id }} className="w-full md:w-auto">
                            <Button size="lg" className="w-full h-14 px-8 bg-success hover:bg-success/90 text-slate-950 font-black uppercase tracking-[0.2em] rounded-none text-xs shadow-[0_0_30px_rgba(34,197,94,0.2)]">
          ...
          <section className="space-y-4">
             <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-cream/40">Upcoming Exchange Windows</h2>
             </div>
                ...
                <table className="w-full text-left border-collapse">
                   <thead className="bg-black/40 border-b border-white/10">
                      <tr className="font-mono text-[9px] uppercase tracking-[0.2em] text-gold-muted/60">
                        ...
                        <th className="px-4 py-4 font-black">Asset Class</th>
                        <th className="px-4 py-4 font-black text-center">Status</th>
                        <th className="px-4 py-4 font-black text-right">Deployment</th>
                             ...
                             daysAway === 0 && "bg-success/[0.02]"
                              ...
                              <td className="px-6 py-5">
                                 <div className="space-y-1">
                                    <div className="text-sm font-bold text-cream uppercase tracking-tight group-hover:text-gold transition-colors">{sale.name}</div>
                                    <div className="text-[9px] font-mono text-cream/20 uppercase tracking-tighter">D{String(sale.day).padStart(3, '0')} · {gameCalendarDate(sale.day)}</div>
                                 </div>
                              </td>
                              <td className="px-4 py-5">
                                 <Badge variant="outline" className="text-[8px] h-4 font-black uppercase tracking-tighter border-white/10 text-cream/40 rounded-none">
                                    {KIND_LABELS[sale.kind as keyof typeof KIND_LABELS] ?? sale.kind}
                                 </Badge>
                              </td>
                              <td className="px-4 py-5 text-center">
                                 {isScheduled ? (
                                    <span className="text-[9px] font-black uppercase text-cream/20 tracking-widest">SCHEDULED</span>
                                 ) : daysAway === 0 ? (
                                    <span className="text-[9px] font-black uppercase text-success tracking-widest animate-pulse">ACTIVE_RING</span>
                                 ) : (
                                    <span className="text-[9px] font-black uppercase text-gold/40 tracking-widest text-center">CATALOG_OPEN</span>
                                 )}
                              </td>
                              <td className="px-4 py-5 text-right font-mono text-xs tabular-nums">
                                 {isScheduled ? (
                                    ...
                                    <div className="flex flex-col items-end">
                                       <span className="text-cream/60">{sale.lots.filter(l => !l.withdrawn).length} LOTS</span>
                                       {playerLots.length > 0 && <span className="text-[8px] text-success font-black uppercase">INC_{playerLots.length}_ASSET</span>}
                                    </div>
                                 )}
                              </td>
                              <td className="px-6 py-5 text-right">
                                 {isScheduled ? (
                                    <Button disabled size="sm" variant="ghost" className="h-8 px-4 text-[9px] font-black uppercase border border-white/5 opacity-20">PENDING</Button>
                                 ) : (
                                    <Link to="/auction/$saleId" params={{ saleId: sale.id }}>
                                       <Button 
                                          ...
                                          className={cn(
                                            ...
                                            daysAway === 0 ? "border-success text-success bg-success/5" : "text-cream/40"
                                          )}
                                       >
