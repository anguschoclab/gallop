...
export const Route = createFileRoute("/german-calendar")({
  component: GermanCalendarPage,
});

function GermanCalendarPage() {
  const [gradeFilter, setGradeFilter] = useState<Grade | "all">("all");
  const [trackFilter, setTrackFilter] = useState<string>("all");
  const races = useGame((s) => s.races);

  const germanRaces = races.filter((race) =>
    race.graded && GERMAN_TRACKS.has(race.graded.track)
  );

  const tracks = Array.from(new Set(germanRaces.map((r) => r.graded?.track))).filter((t): t is string => Boolean(t)).sort();

  const filteredRaces = germanRaces.filter((race) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">German Racing Calendar</h1>
        <p className="text-muted-foreground">Grade 1, 2, and 3 stakes races across Germany</p>
      </div>
