...
type TrackFilter =
  ...
  | "San Sebastián"
  | "Dos Hermanas";
const TRACK_FILTERS: TrackFilter[] = [
  "all",
  "Woodbine",
  "Fort Erie",
  "Century Mile",
...
export const Route = createFileRoute("/races")({
  ...
  ): { grade: GradeFilter; country: CountryFilter; surface: SurfaceFilter; track: TrackFilter } => {
...
function CalendarView({
  ...
  // Group races by day
  const racesByDay = upcoming.reduce(
    (acc: Record<number, Race[]>, race: Race) => {
      if (!acc[race.day]) {
        acc[race.day] = [];
      }
  ...
  const sortedDays = Object.keys(racesByDay)
    .map(Number)
    .sort((a, b) => a - b);
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {racesByDay[raceDay].map((race: Race) => {
              ...
              return (
                <Card
                  key={race.id}
                  className={race.graded ? "border-l-4 border-l-primary" : undefined}
                >
                      ...
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground space-y-1">
                          <div>
                            {race.distance}m ·{" "}
                            {race.graded
                              ? `${race.graded.track} · ${race.graded.surface}`
                              : race.raceClass}
                          </div>
                          <div>
                            Purse{" "}
                          ...
                          <div className="flex items-center gap-2">
                            <span>
                              {race.entries.length}/{race.fieldSize} entered
                            </span>
                            {npcEntryCount > 0 && (
                              <Badge
                                variant="secondary"
                          ...
                          {race.restrictions?.minAge === race.restrictions?.maxAge &&
                            race.restrictions?.minAge !== undefined && (
                              <div>{race.restrictions.minAge}YO only</div>
                            )}
                          {race.restrictions?.minAge !== undefined &&
                            race.restrictions?.maxAge === undefined && (
                              <div>{race.restrictions.minAge}+ YO</div>
                            )}
                        </div>
                      </div>
                      {race.graded && <RaceDetailPanel race={race} />}
                      <BeyerExpectations race={race} horsesMap={horsesMap} />
                      <div className="pt-2 border-t">
                        {ownedEntry ? (
                          <div className="flex flex-col gap-2">
                            <Badge variant="secondary" className="w-fit">
                              {horsesMap.get(ownedEntry.horseId)?.name} entered
                            </Badge>
                            <div className="flex gap-2">
                              ...
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => withdrawRace(race.id, ownedEntry.horseId)}
                              >
                                Withdraw
                              </Button>
                            </div>
...
function RacesPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        (savedGrade && savedGrade !== "all" && GRADE_FILTERS.includes(savedGrade)) ||
        ...
        navigate({
          to: "/races",
          search: {
            grade: savedGrade || "all",
            country: savedCountry || "all",
            surface: savedSurface || "all",
            track: savedTrack || "all",
          },
          replace: true,
        });
      }
  ...
  const horsesMap = useMemo(() => new Map(horses.map((h) => [h.id, h])), [horses]);
  ...
  const filterLabel: Record<GradeFilter, string> = {
    all: "All races",
    G1: "G1 only",
    G2: "G2 only",
    G3: "G3 only",
  };
  const surfaceLabel: Record<SurfaceFilter, string> = {
    all: "All surfaces",
    Turf: "Turf",
    Dirt: "Dirt",
    Synthetic: "Synthetic",
        ...
        <div className="flex gap-3 items-center">
          <Select
            ...
            onValueChange={(c) =>
              navigate({
                to: "/races",
                search: { grade, country: c as CountryFilter, surface, track },
              })
            }
          >
          ...
          <Select
            ...
            onValueChange={(t) =>
              navigate({
                to: "/races",
                search: { grade, country, surface, track: t as TrackFilter },
              })
            }
          >
            <SelectTrigger className="w-[180px]">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                        ...
                        {r?.minAge === r?.maxAge && r?.minAge !== undefined && (
                          <span>{r.minAge}YO only</span>
                        )}
                        {r?.minAge !== undefined && r?.maxAge === undefined && (
                          <span>{r.minAge}+ YO</span>
                        )}
                      </div>
                      {race.graded && <RaceDetailPanel race={race} />}
                      <BeyerExpectations race={race} horsesMap={horsesMap} />
          ...
          <CardContent className="space-y-2">
            {past.map((r) => (
              <div key={r.id} className="flex justify-between text-sm py-1 border-b last:border-0">
                <span>
                  <span className="text-muted-foreground">D{r.day}</span> · {r.name}
                </span>
                <span className="text-muted-foreground">
                  Winner: {horsesMap.get(r.result?.[0]?.horseId ?? "")?.name ?? "—"}
                </span>
              </div>
            ))}
          </CardContent>
...
function EntryPicker({
  ...
  eligible: { id: string; name: string; energy: number }[];
  disabled: boolean;
