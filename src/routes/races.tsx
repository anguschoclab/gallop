      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-success/20 pb-6">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2 font-mono text-[10px] uppercase tracking-widest text-cream/40">
             <span>Active Targets: <NumericValue value={filteredRaces.length} /></span>
             <span className="w-1 h-1 rounded-full bg-white/20" />
             <span>Regional Clusters: <NumericValue value={countries.length} /></span>
             <span className="w-1 h-1 rounded-full bg-white/20" />
        ...
        <div className="flex gap-2">
           <Link to="/calendar">
            <Button variant="outline" size="sm" className="gap-2 border-white/5 hover:bg-white/5 text-cream/40 font-bold uppercase text-[10px] tracking-widest">
        ...
        <aside className="space-y-8 sticky top-6">
          <section className="space-y-4">
             <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-cream/40">Target Filters</h2>
             </div>
             <Card className="bg-slate-900/40 border-white/5 rounded-none shadow-xl border-l-2 border-l-success/40">
                <CardContent className="p-5 space-y-5">
                  <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-black text-success/40 tracking-widest px-1">Search Identifier</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-black text-success/40 tracking-widest px-1">Asset Grade</label>
                    <Select value={grade} onValueChange={(v) => updateFilter("grade", v)}>
                      ...
                      <SelectContent className="bg-slate-950 border-white/10">
                        <SelectItem value="all">ALL_GRADES</SelectItem>
                        <SelectItem value="G1">GRADE_1_ELITE</SelectItem>
                        <SelectItem value="G2">GRADE_2_MID</SelectItem>
                        <SelectItem value="G3">GRADE_3_STD</SelectItem>
                        <SelectItem value="Graded">ALL_GRADED</SelectItem>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-black text-success/40 tracking-widest px-1">Deployment Zone</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-black text-success/40 tracking-widest px-1">Ownership Filter</label>
                      ...
                      {(['all', 'owned', 'others'] as const).map(o => (
                        <button
                          ...
                          className={cn(
                            ...
                            owned === o ? "bg-white/5 border-success text-success" : "border-transparent text-cream/20 hover:text-cream/40"
                          )}
                        >
                          {o === 'all' ? 'GLOBAL_VIEW' : o === 'owned' ? 'MY_DEPLOYMENTS' : 'EXTERNAL_RACES'}
                        </button>
                      ))}
                    </div>
                  </div>
        ...
        <main className="space-y-6">
          <div className="flex items-center justify-between bg-slate-900/40 p-2 border border-white/5 rounded-lg">
             <div className="flex items-center gap-4 px-4 font-mono text-[10px] uppercase font-black tracking-widest text-cream/40">
                ...
                <span>Detected: <NumericValue value={filteredRaces.length} /> MISSION_PARAMETERS</span>
             </div>
             ...
             <div className="flex items-center gap-1">
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn("h-8 px-3 gap-2 uppercase text-[10px] font-black tracking-widest rounded", viewMode === "list" ? "bg-white/10 text-success" : "text-cream/40")}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn("h-8 px-3 gap-2 uppercase text-[10px] font-black tracking-widest rounded", viewMode === "grid" ? "bg-white/10 text-success" : "text-cream/40")}
          ...
          {filteredRaces.length === 0 ? (
            <div className="p-32 text-center border-2 border-dashed border-white/5 bg-black/10">
               ...
               <p className="font-bold text-cream/40 uppercase tracking-[0.3em] font-[family-name:var(--font-display)]">No Signal Detected</p>
               <p className="text-[10px] font-mono text-cream/10 uppercase mt-2">Adjust filter parameters to recalibrate sensors.</p>
            </div>
          ) : viewMode === "grid" ? (
              ...
              {filteredRaces.map((r: Race) => (
                <div key={r.id} className="relative group">
                   <div className={cn(
                      ...
                      r.entries.some(e => e.owned) ? "bg-success" : "bg-white/5 group-hover:bg-success/20"
                   )} />
               ...
               <table className="w-full text-left border-collapse">
                  <thead className="bg-black/40 border-b border-white/10">
                    <tr className="font-mono text-[9px] uppercase tracking-[0.2em] text-success/60">
                      <th className="px-6 py-3 font-black w-1">D.OY</th>
                      <th className="px-4 py-3 font-black">Designation</th>
                      ...
                      <th className="px-6 py-3 font-black text-right">Deployment</th>
                    </tr>
                  </thead>
                        ...
                        <React.Fragment key={r.id}>
                          <tr className={cn(
                            ...
                            <td className="px-4 py-4">
                              <div className="flex flex-col">
                                 <div className="flex items-center gap-2">
                                    <span className="font-bold text-cream uppercase tracking-tight group-hover:text-success transition-colors">{r.name}</span>
                                    {r.graded && (
                                       <Badge variant="outline" className={cn("text-[8px] h-3.5 px-1 font-black rounded-none", 
                                          r.graded.grade === 'G1' ? 'border-fame text-fame' : 'border-success/30 text-success/60'
                                       )}>{r.graded.grade}</Badge>
                                    )}
                                 </div>
                                 <span className="text-[9px] font-mono text-cream/20 uppercase tracking-tighter">{r.raceClass}</span>
                              </div>
                            </td>
                            <td className="px-4 py-4">
                            ...
                            <td className="px-4 py-4 text-right">
                               <span className="font-mono font-bold text-success text-sm tracking-tighter tabular-nums">{formatCurrency(r.purse)}</span>
                            </td>
                            <td className="px-6 py-4 text-right">
                               {isEntered ? (
                                  <Badge className="bg-success text-slate-950 text-[9px] font-black tracking-widest h-6 rounded-none animate-pulse px-3 shadow-[0_0_10px_rgba(34,197,94,0.3)]">DEPLOYED</Badge>
                               ) : (
                                  <Button 
                                    variant="ghost" 
                                    size="sm" 
