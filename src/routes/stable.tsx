...
function StablePage() {
  ...
  const filteredMyHorses = useMemo(() => {
    ...
    else if (status === "active") result = myHorses.filter((h) => h.lifecycleStatus === "active" && !h.consignedSaleId);
    ...
    <div className="space-y-6 pb-20 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gold/20 pb-6">
        ...
        <div className="flex gap-2">
          <Link to="/scheduler">
            <Button variant="outline" size="sm" className="gap-2 border-gold/20 hover:bg-gold/10 text-gold-muted font-bold uppercase text-[10px] tracking-widest">
              <Clock className="h-3.5 w-3.5" />
              Campaign Logs
            </Button>
          </Link>
          <Link to="/stallions">
            <Button variant="outline" size="sm" className="gap-2 border-pink-500/20 hover:bg-pink-500/10 text-pink-400 font-bold uppercase text-[10px] tracking-widest">
      ...
      >
        <div className="flex items-center justify-between bg-slate-900/40 p-1 border border-white/5 rounded-lg">
          <TabsList className="bg-transparent h-9">
            <TabsTrigger value="roster" className="gap-2 uppercase text-[10px] font-black tracking-widest data-[state=active]:bg-gold data-[state=active]:text-slate-950 h-full px-6">
              Main Registry
            </TabsTrigger>
            <TabsTrigger value="rivals" className="gap-2 uppercase text-[10px] font-black tracking-widest data-[state=active]:bg-blue-500 data-[state=active]:text-slate-950 h-full px-6">
              Rival Archives
            </TabsTrigger>
          </TabsList>
          ...
          {tab === "roster" && (
            <div className="flex items-center gap-1 pr-1">
              <Button 
                variant="ghost" 
                size="sm" 
                className={cn("h-7 w-7 p-0 rounded", view === "ledger" ? "bg-white/10 text-gold" : "text-cream/40")}
              ...
              <Button 
                variant="ghost" 
                size="sm" 
                className={cn("h-7 w-7 p-0 rounded", view === "gallery" ? "bg-white/10 text-gold" : "text-cream/40")}
        ...
        <TabsContent value="roster" className="mt-0 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 bg-black/20 p-4 border border-white/5">
            <div className="flex flex-wrap gap-2">
              {(
                [
                  { key: "active", label: "Operational", count: counts.active, icon: Activity, color: "text-success" },
            ...
            <div className="flex items-center gap-2">
               ...
               <Button variant="outline" size="sm" className="h-8 w-8 p-0 border-white/5 text-cream/20 hover:text-cream">
          ...
          {view === "ledger" ? (
                    ...
                    return (
                      <tr key={h.id} className="group hover:bg-white/[0.02] transition-colors relative">
                        ...
                        <td className="px-4 py-4">
                          <Link to="/stable/$horseId" params={{ horseId: h.id }} className="flex items-center gap-3">
                            <HorseBit horse={h} />
                            {h.activeInjury && (
                               <Badge variant="destructive" className="text-[8px] h-3.5 px-1 font-black animate-pulse">INJURED</Badge>
                            )}
                          </Link>
                        </td>
                          ...
                          <div className="flex flex-col items-center gap-1">
                             <div className="w-16 h-1 bg-white/5 rounded-full overflow-hidden">
                               <div className={cn(
                                 ...
                                 h.energy > 60 ? "bg-success" : h.energy > 30 ? "bg-warning" : "bg-destructive"
                               )} style={{ width: `${h.energy}%` }} />
                             </div>
                             <span className="text-[9px] font-mono text-cream/40 uppercase">E:{h.energy}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-4 text-center">
                          <Badge variant="outline" className={cn(
                            "text-[9px] font-black tracking-tighter uppercase h-5",
                            (h.peakingIndex ?? 0) > 20 ? "border-fame text-fame bg-fame/5" : "border-white/10 text-cream/40"
                          )}>
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                             <Link to="/stable/$horseId" params={{ horseId: h.id }} hash="training">
                               <Button variant="ghost" size="sm" className="h-7 w-7 p-0 hover:bg-gold/10 hover:text-gold text-cream/20" title="Training Room">
                                 <Zap className="h-3.5 w-3.5" />
                               </Button>
                             </Link>
                             <Link to="/scheduler">
                               <Button variant="ghost" size="sm" className="h-7 w-7 p-0 hover:bg-blue-400/10 hover:text-blue-400 text-cream/20" title="Mission Plan">
                                 <Clock className="h-3.5 w-3.5" />
                               </Button>
                             </Link>
                             <Link to="/stable/$horseId" params={{ horseId: h.id }}>
                               <Button variant="ghost" size="sm" className="h-7 px-2 font-mono text-[9px] uppercase font-black tracking-tighter text-cream/40 group-hover:text-gold border border-transparent group-hover:border-gold/20">
              ...
              {filteredMyHorses.length === 0 && (
                <div className="p-20 text-center space-y-4 bg-black/10">
                   ...
                   <div className="space-y-1">
                     <p className="text-sm font-bold text-cream/60 uppercase tracking-widest font-[family-name:var(--font-display)]">No Records Located</p>
                     <p className="text-[10px] font-mono text-cream/20 uppercase tracking-tighter">Current filter parameters yielded zero matches in the registry.</p>
                   </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredMyHorses.map((h) => (
                <Link key={h.id} to="/stable/$horseId" params={{ horseId: h.id }} className="group">
                  <div className="relative border border-white/5 bg-slate-900/40 p-4 hover:border-gold/40 transition-all duration-300">
        ...
        <TabsContent value="rivals" className="mt-0 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          {filteredRivalStables.length === 0 ? (
            <div className="p-20 text-center border border-dashed border-white/10 opacity-40">
                <p className="font-mono text-xs uppercase tracking-widest">No Intelligence Data Available</p>
            </div>
          ) : (
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                           <div className="w-8 h-8 rounded-sm rotate-45 border border-white/20 shadow-lg shrink-0"
                      ...
                      <div className="space-y-3">
                         <div className="flex justify-between items-center bg-black/40 p-2 rounded border border-white/5">
                            <span className="text-[9px] font-black uppercase tracking-widest text-cream/40 flex items-center gap-1.5">
                               <Flame className={cn("h-3 w-3", friction > 50 ? "text-red-500" : "text-cream/20")} />
                               Relation
                            </span>
                            <span className={cn("text-[10px] font-bold uppercase", 
                               friction > 50 ? "text-red-400" : friction < -30 ? "text-success" : "text-cream/60"
                            )}>
                         ...
                         <div className="flex justify-between text-xs font-mono">
                            <span className="text-cream/40">Assets: <span className="text-cream">{stableHorseCount}</span></span>
