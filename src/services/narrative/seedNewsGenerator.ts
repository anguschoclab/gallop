...
function buildRivalIntros(
  ...
  for (const stable of eliteMajor) {
    ...
    const bodies = [
      ...
      `Based in ${country}, ${stable.name} comes with high expectations. Their reputation of ${stable.reputation} means they shouldn't be underestimated. ${description}`,
      `A closer look at ${stable.name} reveals a meticulously run operation out of ${country}. Sporting a reputation of ${stable.reputation}, they are primed for battle. ${description}`,
      `The racing scene in ${country} has produced a formidable rival in ${stable.name}. With a solid ${stable.reputation} reputation, they command attention. ${description}`,
      `Operating with a reputation of ${stable.reputation}, ${stable.name} represents the best of ${country}. ${description} They will be a tough nut to crack.`,
      `The story of ${stable.name} continues to unfold in ${country}. Their impressive ${stable.reputation} reputation precedes them. ${description}`,
      `Rival stable ${stable.name} from ${country} is a proven quantity with a reputation of ${stable.reputation}. ${description} Expect them to be a constant thorn in the side.`,
    ];
