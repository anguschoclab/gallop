 ...
 */
export function calculateGeneticCompatibility(sire: Horse, dam: Horse): { score: number; description: string; warning?: string } {
  ...
  const sensoryScore = evaluateGeneticTrait(sireGenetics.sensoryPerception, damGenetics.sensoryPerception);
  score += sensoryScore * 0.25;
  
  // Evaluate signal transduction genes
  const signalScore = evaluateGeneticTrait(sireGenetics.signalTransduction, damGenetics.signalTransduction);
  ...
  if (sire.healthStatus === "covering_sickness" || dam.healthStatus === "covering_sickness") {
    warnings.push("High risk of covering sickness (dourine) transmission - sexually transmitted disease with 50%+ mortality");
  }
 ...
 */
export function calculateBlueHenContribution(dam: Horse): { score: number; description: string; isBlueHen: boolean } {
 ...
 */
export function calculateFounderEffect(sireName: string, damName: string): { score: number; description: string; warning?: string } {
 ...
 */
export function calculateFoundationStockProximity(sireName: string, damName: string): { score: number; description: string } {
  const sire = findHorseByName(sireName);
  const dam = findHorseByName(damName);
  ...
  function checkForFoundationInLine(horse: any, depth: number = 0): void {
    if (depth > 4 || !horse) return;
 ...
 */
export function checkNickingAffinity(sireName: string, damName: string): { hasAffinity: boolean; affinity: number; description: string } {
  const sire = findHorseByName(sireName);
  const dam = findHorseByName(damName);
 ...
 */
export function calculateInbreedingCoefficient(sireName: string, damName: string): { coefficient: number; warning: string } {
  const sire = findHorseByName(sireName);
  const dam = findHorseByName(damName);
 ...
 */
export function calculateDosageCompatibility(sireName: string, damName: string): { score: number; description: string } {
 ...
 */
export function calculateParentPerformance(sire: Horse, dam: Horse): { score: number; description: string } {
  let sireScore = 0;
  let damScore = 0;
 ...
 */
export function calculateConformationCompatibility(sire: Horse, dam: Horse): { score: number; description: string } {
 ...
 */
export function calculateTemperamentCompatibility(sire: Horse, dam: Horse): { score: number; description: string } {
...
export function calculateBreedingCompatibility(sire: Horse, dam: Horse): BreedingCompatibilityResult {
  ...
  const foundationStock = calculateFoundationStockProximity(sire.sireName || "", dam.sireName || "");
  ...
  return {
    ...
    factors: {
      ...
      dosage: dosage,
      inbreeding: { score: inbreedingScore, description: `Coefficient: ${(inbreeding.coefficient * 100).toFixed(1)}%`, warning: inbreeding.warning },
      parentPerformance,
      conformation,
      temperament,
      foundationStock,
