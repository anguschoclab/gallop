    ...
    (set, get) => ({
      ...
      resolveRace: (raceId, result) => {
        ...
        for (const r of result) {
          ...
          h.raceHistory = [{ raceId, raceName: race.name, position: r.position, day: s.day, beyer, grade: race.graded?.grade, distance: race.distance, surface: race.surface, purse: race.purse, fieldSize: result.length }, ...h.raceHistory].slice(0, 50);
          // form change
