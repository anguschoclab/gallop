...
exports.calculateDosageProfile = calculateDosageProfile;
exports.calculateDosageIndex = calculateDosageIndex;
exports.calculateCenterOfDistribution = calculateCenterOfDistribution;
exports.generatePedigree = generatePedigree;
exports.calculateDosageMetrics = calculateDosageMetrics;
exports.interpretDosageIndex = interpretDosageIndex;
 ...
 */
function calculateDosageProfile(pedigree) {
    var profile = {
        brilliant: 0,
        intermediate: 0,
        classic: 0,
    ...
    for (var _i = 0, pedigree_1 = pedigree; _i < pedigree_1.length; _i++) {
        var node = pedigree_1[_i];
        var horse = (0, pedigreeData_1.findHorseByName)(node.name);
        if (horse === null || horse === void 0 ? void 0 : horse.dosageGroups) {
            var points = GENERATION_POINTS[node.generation] || 0;
            // If the horse has one dosage group, assign all points to it
            if (horse.dosageGroups.length === 1) {
                var group = horse.dosageGroups[0];
                profile[group.toLowerCase()] += points;
            }
            // If the horse has two dosage groups, split points equally
            else if (horse.dosageGroups.length === 2) {
                var halfPoints = points / 2;
                profile[horse.dosageGroups[0].toLowerCase()] += halfPoints;
                profile[horse.dosageGroups[1].toLowerCase()] += halfPoints;
            }
        }
    }
    return profile;
 ...
 */
function calculateDosageIndex(profile) {
    var numerator = profile.brilliant + profile.intermediate + 0.5 * profile.classic;
    var denominator = 0.5 * profile.classic + profile.solid + profile.professional;
    if (denominator === 0) {
        return Infinity;
    }
 ...
 */
function calculateCenterOfDistribution(profile) {
    var totalPoints = profile.brilliant +
    ...
    var numerator = 2 * profile.brilliant + profile.intermediate - profile.solid - 2 * profile.professional;
 ...
 */
function generatePedigree(sireName) {
    var pedigree = [];
    if (!sireName) {
        return pedigree;
    }
    ...
    if (sire) {
        ...
        if (sire.sire) {
            var grandsire = (0, pedigreeData_1.findHorseByName)(sire.sire);
            if (grandsire) {
                pedigree.push({ name: grandsire.name, generation: 2 });
                // Generation 3: Great-grandsire
                if (grandsire.sire) {
                    var greatGrandsire = (0, pedigreeData_1.findHorseByName)(grandsire.sire);
                    if (greatGrandsire) {
                        pedigree.push({ name: greatGrandsire.name, generation: 3 });
                        // Generation 4: Great-great-grandsire
                        if (greatGrandsire.sire) {
                            var greatGreatGrandsire = (0, pedigreeData_1.findHorseByName)(greatGrandsire.sire);
...
function calculateDosageMetrics(sireName) {
    ...
    var cached = DOSAGE_CACHE.get(sireName);
    if (cached)
        return cached;
    var pedigree = generatePedigree(sireName);
    var dosageProfile = calculateDosageProfile(pedigree);
    var dosageIndex = calculateDosageIndex(dosageProfile);
    var centerOfDistribution = calculateCenterOfDistribution(dosageProfile);
    var result = {
        pedigree: pedigree,
        dosageProfile: dosageProfile,
        dosageIndex: dosageIndex,
        centerOfDistribution: centerOfDistribution,
    };
 ...
 */
function interpretDosageIndex(dosageIndex) {
