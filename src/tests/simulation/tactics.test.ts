...
describe('Advanced AI Tactics', () => {
  ...
  const mockJockey = (skill: number): Jockey => ({
    id: 'j1',
    name: 'Jockey',
    stats: { pacing: skill, positioning: skill, vigor: skill, gateSkill: skill, temperament: skill },
    archetype: 'versatile',
  } as any);
  ...
  it('should apply traffic penalties when horses are blocked', () => {
    ...
    const runners = [
      { horseId: 'H1', position: 10, velocity: 16, lane: 0, runningStyle: 'E', finishTime: null, horse: mockHorse('H1', 'E'), jockey: mockJockey(80) },
      { horseId: 'H2', position: 9.5, velocity: 18, lane: 0, runningStyle: 'P', finishTime: null, horse: mockHorse('H2', 'P'), jockey: mockJockey(80) }
    ] as any;
    ...
    if (blockingHorse.position > r2.position && blockingHorse.position - r2.position < 1.5 && Math.abs(blockingHorse.lane - r2.lane) < 0.4) {
