...
describe('Dosage Calculations', () => {
  ...
  describe('calculateDosageProfile', () => {
    it('should calculate points correctly for single and dual groups across generations', () => {
      vi.spyOn(pedigreeData, 'findHorseByName').mockImplementation((name) => {
        ...
        if (name === 'Grandsire') return { name: 'Grandsire', dosageGroups: ['Classic', 'Solid'] } as any;
        if (name === 'GreatGrandsire') return { name: 'GreatGrandsire', dosageGroups: ['Professional'] } as any;
        return undefined;
      });
    ...
    it('should handle empty pedigree', () => {
      ...
      expect(profile).toEqual({
        brilliant: 0, intermediate: 0, classic: 0, solid: 0, professional: 0
      });
    });
  });
  ...
  describe('calculateDosageMetrics', () => {
    it('should calculate full metrics and use cache', () => {
      const mockFindHorse = vi.spyOn(pedigreeData, 'findHorseByName').mockImplementation((name) => {
        if (name === 'SireA') return { name: 'SireA', sire: 'SireB', dosageGroups: ['Classic'] } as any;
  ...
  describe('interpretDosageIndex', () => {
    it('should interpret various index ranges correctly', () => {
      expect(interpretDosageIndex(Infinity)).toBe('Extreme speed preference (very short distances)');
      expect(interpretDosageIndex(undefined)).toBe('Extreme speed preference (very short distances)');
      expect(interpretDosageIndex(4.5)).toBe('High speed preference (short to medium distances)');
      expect(interpretDosageIndex(3.0)).toBe('Above-average speed preference (short to medium distances)');
