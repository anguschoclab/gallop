...
describe("calculateHeadToHeadOdds (lightweight)", () => {
  ...
  it("surface aptitude affects win probability", () => {
    const h1 = mkHorse({
      id: asHorseId("h1"),
      surfaceAptitude: { Turf: 1.2, Dirt: 0.8, Synthetic: 0.95 },
    });
    const h2 = mkHorse({
      id: asHorseId("h2"),
      surfaceAptitude: { Turf: 0.8, Dirt: 1.2, Synthetic: 0.95 },
    });
    const results = calculateHeadToHeadOdds([h1, h2], 1600, "Turf");
    const r1 = results.find((r) => r.horseId === "h1")!;
    const r2 = results.find((r) => r.horseId === "h2")!;
  ...
  it("distance aptitude affects win probability", () => {
    const h1 = mkHorse({ id: asHorseId("h1"), distanceAptitude: 1600 });
    const h2 = mkHorse({ id: asHorseId("h2"), distanceAptitude: 2400 });
    const results = calculateHeadToHeadOdds([h1, h2], 1600, "Turf");
    const r1 = results.find((r) => r.horseId === "h1")!;
    const r2 = results.find((r) => r.horseId === "h2")!;
    expect(r1.winPct).toBeGreaterThan(r2.winPct);
  ...
  it("handles 3 horses", () => {
    const h1 = mkHorse({ id: asHorseId("h1") });
    const h2 = mkHorse({ id: asHorseId("h2") });
    const h3 = mkHorse({ id: asHorseId("h3") });
