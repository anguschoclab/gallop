...
import { computeGenomeModifiers, classifyCoi, inbreedingPerformanceDampener, computeProspectiveCoi, detectInbreedingPattern, computeAhc, computeCoiFromSnapshot } from "@/core/breeding/populationGenetics";
...
describe("populationGenetics", () => {
    ...
    describe("computeGenomeModifiers", () => {
        ...
        it("applies close-inbreeding modifiers with high prepotency and significant depression penalty", () => {
            const modifiers = computeGenomeModifiers(0.10, 0.0);
        ...
        it("reduces depression penalty based on AHC (Ancestral History Coefficient)", () => {
            const noRelief = computeGenomeModifiers(0.10, 0.0);
            const highRelief = computeGenomeModifiers(0.10, 1.0);
    ...
    describe("computeAhc", () => {
        it("calculates AHC based on parent AHC and career wins", () => {
            ...
            const horseMap = new Map([["sire", sire], ["dam", dam]]);
    ...
    describe("computeProspectiveCoi", () => {
        it("returns 0 if there are no common ancestors", () => {
            const sire = createTestHorse({ id: "sire", pedigree: { sireId: "sireSire", damId: "sireDam", generation: 1, name: "Sire" } as any });
            const dam = createTestHorse({ id: "dam", pedigree: { sireId: "damSire", damId: "damDam", generation: 1, name: "Dam" } as any });
        ...
        it("calculates a non-zero COI for a mating with a common ancestor", () => {
            ...
            const sire = createTestHorse({
                id: "sire",
                pedigree: {
                    sireId: commonAncestorId, damId: "sireDam", generation: 1, name: "Sire"
                } as any
            });
            const dam = createTestHorse({
                id: "dam",
                pedigree: {
                    sireId: commonAncestorId, damId: "damDam", generation: 1, name: "Dam"
                } as any
            });
        ...
        it("applies lower weights for distant generations", () => {
            ...
            const sire = createTestHorse({
                ...
                pedigree: {
                    ...
                    name: "Sire",
                    sirePedigree: {
                         sireId: "s2", damId: "d2", generation: 2,
                         sirePedigree: { sireId: "s3", damId: "d3", generation: 3,
            ...
            const dam = createTestHorse({
                ...
                pedigree: {
                    ...
                    name: "Dam",
                    damPedigree: {
                        sireId: "s2b", damId: "d2b", generation: 2,
                        sirePedigree: { sireId: "s3b", damId: "d3b", generation: 3,
