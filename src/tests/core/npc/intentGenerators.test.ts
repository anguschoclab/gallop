...
describe("generateNpcIntents auction consignment", () => {
  ...
  // Note: Testing auction weight = 0 through generateNpcIntents is not feasible
  // without mocking the coordination layer, because coordinateSubsystems
  // overwrites stableAI.subsystemWeights with computed weights.
  // The weight = 0 behavior is verified at the unit level in auctionAI.test.ts
  // (shouldConsignHorse weight modulation > returns false when weight = 0).
});
