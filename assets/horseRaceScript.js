...
function GetVersionNumber() { return g_SIMHorseRaceViewer_Version; }
...
{
    if (g_RaceTimer.bIsRunning == false)
        startRace();
    else
        pauseRace();
}
...
function loadRaceData(id) {
    ...
    if (raceData.Type == "harness")
        g_nHorseWidth = g_nHorseHarnessWidth;
...
function setupCanvas () {
    ...
    jQuery('#skyGraphic').css('background', 'transparent url(' + imagePath(raceData.SkyGraphic) + ') repeat-x');
    ...
    jQuery('#fenceGraphic').css('background', 'transparent url(' + imagePath(raceData.FenceGraphic) + ') repeat-x');
    ...
    jQuery('#trackGraphic').css('background', 'transparent url(' + imagePath(raceData.SurfaceGraphic) + ') repeat-x');
    ...
    jQuery('#trackObstacle').css('background', 'transparent url(' + imagePath(markerGraphic) + ') repeat-x');
...
function updateRaceTickers () {
    ...
    if (curr > g_RaceTimer.fRaceDuration)
        curr = g_RaceTimer.fRaceDuration;
    
    if (dist > g_RaceTimer.fRaceDistance)
        dist = g_RaceTimer.fRaceDistance;

    var clock = curr.ClockFormatting();
    var sloMo = "";
    if (g_RaceTimer.fRate <= 0.5)
        sloMo = " [SloMo]";
    if (g_RaceTimer.fRate >= 4.0)
        sloMo = " [4X]";
...
function TimerCallback() {
    ...
    else
    {
        ...
        if (g_RaceTimer.ElapsedPercent() >= 1.0)    // means lead horse has crossed
...
//// Member Variables & Initialization ////
    ...
    methods.nFocalPointRightX = methods.nTrackWidth - (methods.nLaneRightInset + methods.nFocalPointInsetRight);
...
//// Functions ////
    methods.setFocalPointForTime = function(time, dist) {
        ...
        if (dist == 0)  // default behavior based on race average speed, not used
            methods.nFocalPointDistance = g_RaceTimer.DistanceAtTime(time);
        
        methods.nFocalPointX = ((time/g_RaceTimer.fRaceDuration) * methods.nFocalWidth) + methods.nFocalPointLeftX;
    ...
    methods.setupLanes = function(numLanes) {
        ...
        if (laneHeight < methods.nLaneHeight)
            methods.nLaneHeight = laneHeight;
 ...
 //       methods.nTrackHeight = (numLanes * methods.nLaneHeight) + 10;
    ...
    methods.startupAnimations = function() {
        ...
        g_Turf.css({ top: methods.nTrackTop, left: 0, height: methods.nTrackHeight, width: 10,'opacity': 0.0 }); // starting point before animation
        g_Marker.css({top: methods.nTrackTop, height: methods.nTrackHeight, left: 700, width: 40, opacity: 1.0 }); // starting point before animation

        g_Marker.show();

        g_Turf.animate({ top: methods.nTrackTop, left: 0, height: methods.nTrackHeight, width: methods.nTrackWidth, opacity: .9}, g_RaceSetupAnimationSpeed);
    ...
    methods.setCurrentCheckpoint = function(dist) {
        ...
        for (var iter = methods.Checkpoints.length-1; iter >= 0; iter--)    // reverse iteration
        {
            if (dist <= methods.Checkpoints[iter])
                methods.currentCheckpoint = methods.Checkpoints[iter];
            else
                break;
        }
    ...
    methods.chooseCheckpointGraphic = function() {
        ...
        else if (raceData.Steeple == 1)
            methods.setCheckpointGraphic( imagePath('markers-steeple.png'));
        else
            methods.setCheckpointGraphic( imagePath('markers-furlong.png'));
    }
    ...
    methods.drawCheckpoint = function () {
        ...
        if (xx < -50) {  // checkpoint is going all the way off the screen left
            methods.setCurrentCheckpoint(methods.nFocalPointDistance);  // get next one ready
        }
        
        if (g_Marker && xx < g_nCanvasWidth) {  // will be on screen so start animating
            if (xx > g_Marker.css('left'))
                xx = g_Marker.css('left');
            g_Marker.show();
...
function CRaceHorse(entry, lane) {
    ...
    methods.setHorseGraphic = function(URL) {
        ...
        methods.horseSprite = methods.horseSprite.sprite({  fps: 2,
                                                            no_of_frames: 6,
                                                            start_at_frame: 1,
    ...
    methods.checkForJump = function() {
        ...
        if (g_RaceTrack.checkpointIsJump() == false)
            return;
        ...
        if (pixelsToTakeoff > 60)   // we are not close enough to jump, so adjust the animation rate up or down
        {
...
//console.log('Just testing logging');
    ...
    methods.switchToState = function(newState) {
        ...
        case kHorseState_StartLine:
            ...
            methods.title.animate({ left: g_RaceTrack.nLaneLeftInset, opacity: 1.0 }, g_RaceSetupAnimationSpeed);
            methods.scoreText.hide();
...
function animateHorses(nextRaceTime, animateInterval) {
    if (animateInterval == 'undefined')
        animateInterval = g_RaceTimer.nAnimationIntervalMilliseconds;
    
    var leadHorseDist = 0.0;
    var leadHorse = calcHorseDistancesReturnLeadHorse(nextRaceTime);
    if (leadHorse)
        leadHorseDist = leadHorse.distanceSinceStart;
    ...
    if (CheckStateForAllHorses(kHorseState_CrossFinish))
        SetStateForAllHorses(kHorseState_Done);
