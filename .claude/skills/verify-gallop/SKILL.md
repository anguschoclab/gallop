...
2. **`src/tests/smoke/routeMount.smoke.test.tsx`** — auto-discovers and mounts **every** route component (46 component routes + 16 redirect-only routes) via `import.meta.glob` against the real store with a mocked router. Catches loops anywhere in the route tree, not just in pre-selected components. Uses `src/test-utils/routerMock.tsx` (complete mock with `createFileRoute`, `createRootRoute`, `notFound`, `Navigate`, etc.) and `midGameSeed()` (minimal non-empty state). Routes that `throw notFound()` for non-existent dummy IDs are treated as passes (the component mounted and ran to the guard). `__root.tsx` is skipped (needs a real router). Run: `bunx vitest run src/tests/smoke/routeMount.smoke.test.tsx`.
