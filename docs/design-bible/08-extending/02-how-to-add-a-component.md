...
- Make props minimal and explicit. Pass the full domain object (`horse: Horse`) when the component is _about_ that object; pass only what you need otherwise.
...
**Rule of thumb:** if the new behaviour is a _style variant_ of the existing component, extend it. If it's a _semantic difference_, create a new component.

---

## Anti-patterns

- **Don't** create a `<HorsePicker />` if you already have a `<Combobox />` with horse data — that's a _use case_, not a component.
...
We never deprecate a component while keeping the old one. _One way to do a thing_ is a feature.
