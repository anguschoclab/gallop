...
| **Semantic** | `--muted` | A primitive given a *role*. Used in `@theme inline` to expose Tailwind utilities. |
| **Tailwind utility** | `bg-muted`, `text-muted-foreground` | What components actually consume. |

**Rule:** components consume *Tailwind utilities*, never primitives. If you find a hex code or `oklch(...)` in a component, file it as a bug. The only exception is the race screen's emerald palette — and that's an explicit gap to be closed (see [05-theming.md](05-theming.md)).
...
- Should we expose a *neutral* slate scale (50–900) for advanced cases, or keep semantic-only?
