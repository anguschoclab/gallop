...
const RESEARCHED_TRACK_DATA: Record<string, { 
  courses: Array<{ 
    surface: string; 
  ...
  "Oi": {
    courses: [
      { surface: "Dirt", circumference: 1400, straightLength: 300, width: 18, name: "Oi Racecourse" }
    ],
  ...
  "Ohi": {
    courses: [
      { surface: "Dirt", circumference: 1400, straightLength: 300, width: 18, name: "Ohi Main Course" },
      { surface: "Dirt", circumference: 1200, straightLength: 250, width: 16, name: "Ohi Inner Course" }
    ],
  ...
  "Newcastle": {
    courses: [
      { surface: "Turf", circumference: 1750, straightLength: 340, width: 20, name: "High Gosforth Park" },
      { surface: "Synthetic", circumference: 1750, straightLength: 340, width: 20, name: "Tapeta Track" }
    ],
  ...
  "Kempton": {
    courses: [
      { surface: "Turf", circumference: 2000, straightLength: 400, width: 22, name: "Kempton Park" },
      { surface: "Synthetic", circumference: 1609, straightLength: 366, width: 20, name: "All-Weather Track" }
    ],
...
function run() {
    ...
    if (researchData) {
      ...
      researchData.courses.forEach((researchCourse, index) => {
        ...
        if (course) {
          ...
          course.sections = generateOvalSections(researchCourse.circumference, researchCourse.straightLength);
        }
      });
      ...
      console.log(`✅ ${track.name} - ${firstCourse.circumference}m circ, ${firstCourse.straightLength}m straight`);
  ...
  // Final stats
  const finalStats = tracks.reduce((acc: any, t: any) => {
  ...
  console.log(`Complete with sections: ${finalStats.complete} (${Math.round(finalStats.complete/tracks.length*100)}%)`);
  console.log(`Still missing: ${finalStats.missing} (${Math.round(finalStats.missing/tracks.length*100)}%)`);
}
