...
function calculateSections(geom: any[]) {
  ...
  for (let i = 0; i < geom.length - 1; i++) {
    const n1 = geom[i], n2 = geom[i + 1], n3 = geom[i + 2] || geom[0];
...
async function discoverCountry(countryName: string) {
  ...
  try {
    ...
    for (const way of ways) {
      ...
      const straightLength = Math.round(Math.max(...sections.filter(s => s.type === "straight").map(s => s.length), 300));
...
async function run() {
  const countries = ["United Kingdom", "Ireland", "France", "United States", "Australia", "Japan", "United Arab Emirates", "Hong Kong", "South Africa"];
